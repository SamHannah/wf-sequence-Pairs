...
sequence<- function(SF1, dp, L, A, N_subjects){
  ...
  LFind<- indices[ indices %% 2 == 1]
  HFind<- indices[ indices %% 2 == 0]  
  pairOrder<- vector(, N_test/N_seqSegment); testOrder<- vector(, N_test)
  ...
  for (N_reps in 1: N_subjects) {
    ...
    ######### test trial pairs consisting of  lf (old/new) and hf (old/new) trials, 
    ...
    ########## test array ##################
    # Make an array of individual test items by randomly combining trial pairs
    # This ensures random ordering of trials (outside of pairs), while ensuring 
    # that there are an equal number of test items in each test condition 
    # weave together pairs from quartet arrays, and form testOrder vector (coding for individual trial conditions)  from pairOrder vector         
    #test PAIR order: 1 = HnLn, 2 = HnLo, 3 = HoLn, 4 = HoLo
    # test ITEM order: 1= Low old, 2 = High old, 3 = Low new, 4 = High new 
    pairOrder<- c(sample(rep(seq(1:N_types), each = N_trialPres)), sample(rep(seq(1:N_types), each = N_trialPres)),
                  sample(rep(seq(1:N_types), each = N_trialPres)), sample(rep(seq(1:N_types), each = N_trialPres)) )
    j<-1; k<- 1; m<- 1; n<- 1;  v<- 1
    for (i in 1:(N_test/2) )  {
      switch(pairOrder[i], 
             "1" = {testItems[j:(j+1), ]<- Qhnln[k:(k+1), ]
                    testOrder[j]<- 3                                 
                    testOrder[(j+1)]<-  4                
                    k<- k+2 },
             "2" = {testItems[j:(j+1), ]<- Qhnlo[m:(m+1), ]
                    testOrder[j]<- 1    
                    testOrder[(j+1)]<- 4
                    m<- m+2 },
             "3" = {testItems[j:(j+1), ]<- Qholn[n:(n+1), ]
                    testOrder[j]<- 3    
                    testOrder[(j+1)]<- 2
                    n<- n+2 },
             "4" = {testItems[j:(j+1), ] = Qholo[v:(v+1), ]
                    testOrder[j]<- 1    
                    testOrder[(j+1)]<- 2
                    v<- v+2 }
      )
    ...
    for (j in 1:N_test) {
      ...
      # construction of presentation pairs: used to evaluate performance for 10  LF-HF pairs per quarter: w indexes quarters
      w<- 0
      switch(testOrder[j],
             "1"={},
             "2"={
               if (j > 2 && testOrder[j-1] == 1) {
                 HOLO<- HOLO+1
                 w<- ceiling(HOLO/N_trialPres)                                    # N_trialPres = number of pairs/quarter = 10
               }else{
                 HOLN<- HOLN+1
